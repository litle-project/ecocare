							...
							<div class="portlet-body">										
								<table class="table table-striped table-bordered table-hover display">
									...
									<?php
								        ...
								        $month1 = date('m', $ts1);
								        $month2 = date('m', $ts2);
								        $date = (($year2 - $year1) * 13) + ($month2 - $month1);
										$no=1;
										for($i=0; $i <count($package) ; $i++){
											if($package[$i]['service_period'] == $data[0]['month'] || $package[$i]['service_period'] == 1 && $schedule == 2){
									?>
										<tr class="gradeX">
											<td><?php echo $no;?></td>
											<td><?php echo $package[$i]["product_name"];?></td>
											<td><?php echo $package[$i]["category_name"];?></td>
											<td><?php echo $package[$i]['total_per_service'];?></td>
											<td><?php echo $package[$i]['product_qty']?></td>
											<td><?php echo $package[$i]['total_per_service'];?></td>
										</tr> 
									<?php  }elseif($schedule == 1){ ?>
										<tr class="gradeX">
											<td><?php echo $no;?></td>
											<td><?php echo $package[$i]["product_name"];?></td>
											<td><?php echo $package[$i]["category_name"];?></td>
											<td><?php echo $package[$i]['total_per_install'];?></td>
											<td><?php echo $package[$i]['product_qty']?></td>
											<td><?php echo $package[$i]['total_per_install'];?></td>
										</tr>
									<?php }$no++;}?>
