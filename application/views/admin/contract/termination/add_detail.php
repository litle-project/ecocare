	...
	<div class="col-lg-12 col-md-12">
		...
		<div class="col-lg-5 col-md-5 col-sm-5">
			<ul class="breadcrumb pull-right">
				<li>
					<a href="#" onclick="window.location.href='<?php echo site_url($this->uri->segment(1));?>'">Manage Contract Request</a>
				</li>
				<li>
			...
			<div class="row">
				<div class="form-body col-md-12">
                    <div class="form-group">
						...
						<div class="col-md-10">
							<div class="col-md-10">
								<input type="text" class="form-control" placeholder="Select Request Date" name="terminate_date" readonly id="datepicker1" required/>
							</div>
						</div>
					</div>
                    					<!-- start teknisi div -->
							...
							<table class="table table-striped table-bordered table-hover display">
								<thead>
									<tr>
										...
										<th>Number Of Product Request</th>
									</tr>
								</thead>
								...
								?>
									<tr class="gradeX">
										...
										<td>
											<?php 
												echo form_input("product_package_qty".$product[$i]["product_id"],"","class='form-control' id='product".$product[$i]["product_id"]."' required='required' disabled");	
											?>
										</td>
									</tr>
								<?php $no++; } ?>
							...
							<table class="table table-striped table-bordered table-hover display">
								<thead>
									<tr>
										...
										<th>Check</th>
										<th>Package Name</th>
										<th>Number Of Package Request</th>
									</tr>
								</thead>
								<tbody class="selected_only">
								<?php
								...
								?>
									<tr class="gradeX">
										...
										<td>
											<input type="number" name="<?php echo "package_qty".$package[$i]["package_id"]; ?>" value="<?php echo $package[$i]['product_package_qty']; ?>" required disabled id="<?php echo "package".$package[$i]["package_id"]?>" class="form-control" max="<?php echo $package[$i]['product_package_qty']; ?>" min='0'>
										</td>
									</tr>
								<?php $no++; } ?>
								</tbody>
...
<script>
	...
	function input_price(checkbox_value2) {
		if(checkbox_value2.checked) {
			...
			$('#contract_detail_id'+checkbox_value2.value).removeAttr('disabled');
			$('#package_price'+checkbox_value2.value).removeAttr('disabled');
		}else{
			$('#package'+checkbox_value2.value).attr('disabled','disabled');
			$('#contract_detail_id'+checkbox_value2.value).attr('disabled','disabled');
			$('#package_price'+checkbox_value2.value).attr('disabled','disabled');
		}
	}
</script>