...
class Tambah_barang extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model("tambah_barang_model");
	...
	{
		priv('view');
		$data["data"] 	= $this->tambah_barang_model->get_data();
		// echo "<pre>"; print_r($data['data']); die();
		$data["page"]	= "inventory/tambah_barang/view";
	...
	public function detail()
	{
		...
		$data['data'] 	= $this->tambah_barang_model->get_data_detail($inventory_id, $branch_id, $gudang_id, $product_id);
		$data["page"]	= "inventory/tambah_barang/detail";
		$data["title"]	= "Detail Log Inventory";
	...
	{
		priv('add');
		$data["page"]		="inventory/tambah_barang/add";
		...
		if($this->input->post()){
			...
			if(empty($check)){
				...
				// input log
				$log['inventory_id'] 	= $id;
				$log['insert_from']		= '2'; // 1 = inv, 2 = tbh brg, 3 = return brang 
