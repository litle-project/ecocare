	...
	function menu($id="",$id2="")
	{
		...
		foreach($query->result_array() as $row){
			...
			$sql2="select * from treatment_price as a
				...
				where  a.treatment_id='".$row["treatment_id"]."'";
			if($this->session->userdata("user_group_id")>1){
				$sql2 .=" AND a.branch_id='".$this->session->userdata("branch_id")."'";
			}
			else{
				if($id2==""){
					$sql2 .=" AND a.branch_id='0'";
				}
				else{
					$sql2 .=" AND a.branch_id='".$id2."'";
				}
